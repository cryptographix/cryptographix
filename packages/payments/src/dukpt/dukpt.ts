...
export class DUKPT {
  ...
  async deriveDUKPTSessionKey(
    ...
    if (bdk.data.k.length !== 16 || ksn.length !== 20) {
  ...
  static async encryptTDES(key: ISymKey, data: ByteArray, encryptTrueFalse) {
    ...
    try {
      ...
      return DUKPT._des(key, padded, encryptTrueFalse, CBC, DUKPT.zeroIV);
    } catch (e) {
      throw e;
    }
  }
  ...
  async encryptDUKPT(
    ...
    if (
      key.data.k.length === 16 &&
      options.encryptionMode.toUpperCase() !== "AES"
    ) {
  ...
  async decryptDUKPT(
    ...
    if (
      key.data.k.length === 16 &&
      options.decryptionMode.toUpperCase() !== "AES"
    ) {
      key.data.k = DUKPT._EDE3KeyExpand(key.data.k);
    }
  ...
  static async _createIPEK(bdk: ISymKey, ksn: string): Promise<ISymKey> {
    ...
    let maskedKSN = ByteArray.and(H2BA("FFFFFFFFFFFFFFE00000"), H2BA(ksn));

    maskedKSN = maskedKSN.slice(0, 8); // take 1st 8 bytes only

    // get LEFT half of IPEK
    const leftIPEK = await DUKPT._des(
      key,
      maskedKSN,
      true /* encrypt */,
      CBC,
    ...
    const rightIPEK = await DUKPT._des(
      key,
      maskedKSN,
      true /* encrypt */,
      CBC,
  ...
  static _getCounter(ksn: ByteArray): number {
    ...
    const integerValue =
      (tailbytes[0] << 16) + (tailbytes[1] << 8) + tailbytes[2];
  ...
  static async _encryptRegister(key: ISymKey, reg: ByteArray) {
    ...
    const desEncrypted = await DUKPT._des(
      top8Key,
      bottom8xorKSN,
      true /* encrypt */,
      CBC,
  ...
  static encryptAES(key: ISymKey, data: ByteArray) {
    ...
    while (data.length % 16) {
      data = ByteArray.concat([data, ByteArray.from([0])]);
    } // pad with zeroes
